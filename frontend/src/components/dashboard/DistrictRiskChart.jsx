...
export const DistrictRiskChart = ({ data = [], onSelectDistrict = null }) => {
  return (
    <div className="bg-white rounded-3xl border border-slate-200/80 p-7 shadow-xs flex flex-col justify-between">
      <div>
        ...
        <div className="mt-5 space-y-2">
          {data.map((item, idx) => (
            <div
              key={item.district}
              className="p-3 rounded-2xl border border-transparent hover:border-slate-200 hover:bg-slate-50/70 transition-all flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-0"
            >
      ...
      <div className="mt-6 pt-4 border-t border-slate-100 text-xs text-slate-500 flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-0">
